...
//import com.wincere.lamda.storm.bolt.SimpleMongoBolt;
import com.wincere.lamda.storm.bolt.DataPoints;
import com.wincere.lamda.storm.bolt.DataPointsMongo;
import com.wincere.lamda.storm.bolt.MasterTAbleSimpleMongoBolt;
import com.wincere.lamda.storm.bolt.QueryMongoBolt;
import com.wincere.lamda.storm.bolt.SimpleMongoBolt;
...
public class TopologyRunnerQuery {
	...
	public static void main(String ar[]){
		...
		builder.setBolt("MongoBolt", new QueryMongoBolt("172.16.1.171", 27017, "mock6", "mastertable5")).shuffleGrouping("kafka-spout");
		builder.setBolt("MongoBoltData", new DataPoints("172.16.1.171", 27017, "mock6", "mastertable5")).shuffleGrouping("kafka-spout");
		//Configuration
		...
		// conf.put(Config.NIMBUS_HOST, "172.16.1.219");
		...
		conf.put("kafka.spout.topic", "test18"); // This was not being read from the property 
		conf.put("kafka.spout.fetch.size","4000000");
		conf.put("kafka.spout.buffer.size.max", "1"); // This is too aggressive and unrealstic.	
		conf.setDebug(true);
		//Topology run
		conf.put(Config.TOPOLOGY_MAX_SPOUT_PENDING, 1);
		//conf.put("spoutConfig.scheme","RawMultiScheme()");
		LocalCluster cluster = new LocalCluster();
		cluster.submitTopology("Vertica-Storage-Toplogie", conf, builder.createTopology());
		Utility.delay(10000);
		cluster.shutdown();
		 
		/*System.setProperty("storm.jar", "/home/neeraj/Developement/storm_separate/target/storm_separate-1.0-SNAPSHOT-jar-with-dependencies.jar");
		try {
			StormSubmitter.submitTopology("Vertica-Storage-Toplogie3", conf, builder.createTopology());
